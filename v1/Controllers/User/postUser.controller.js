...
module.exports = {
  signup: async (req, res) => {
    ...
    const customer = await stripe.customers.create({
      name: formDisplayName(firstName, lastName),
    })
    ...
    const user = new User({
      ...
      data: {
        firstName,
        lastName,
        displayName: formDisplayName(firstName, lastName),
        email,
  ...
  forgotPassword: async (req, res) => {
    ...
    if (!user) {
      return res
        .status(401)
        .send(error('Check your email for password reset link', res.statusCode))
    }
