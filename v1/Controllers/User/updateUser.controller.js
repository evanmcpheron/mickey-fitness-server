...
module.exports = {
  user: async (req, res) => {
    ...
    res
      .status(200)
      .send(success('Successfully updated user', res.statusCode, user))
  },
  profileImageUpload: async (req, res) => {
    console.log(
      '🚀 ~ file: updateUser.controller.js ~ line 7 ~ req.currentUser: ',
      req.currentUser
    )
    if (!req.currentUser) {
      return res
    ...
    res.status(200).send(
      success('Profile photo successfully updated.', res.statusCode, {
        uuid: user._id,
        fileName: user.data.photoURL,
        user,
      })
    )
  },
